...
pub fn serialize_mob_result_zero_copy(
    ...
    cur.write_i32::<LittleEndian>(disable_len).map_err(|e| e.to_string())?;
    
    for id in &result.mobs_to_disable_ai {
        cur.write_u64::<LittleEndian>(*id).map_err(|e| e.to_string())?;
    }
    
    // Write simplify list
    let simplify_len = result.mobs_to_simplify_ai.len() as i32;
    cur.write_i32::<LittleEndian>(simplify_len).map_err(|e| e.to_string())?;
