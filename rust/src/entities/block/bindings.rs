...
use crate::entities::block::processing::process_block_entities_binary_batch;
...
fn get_logger() -> &'static JniLogger {
    LOGGER.get_or_init(|| JniLogger::new("block"))
}
    ...
    match crate::block::processing::process_block_entities_json(&input_str) {
        ...
        Err(e) => {
            jni_log_error!(
                ...
                &mut env,
                "JNI",
                &format!("process_block_entities_json failed: {}", e)
            );
    ...
    match process_block_entities_binary_batch(slice) {
        ...
        Err(e) => {
            jni_log_error!(
                ...
                &mut env,
                "JNI",
                &format!("process_block_entities_binary_batch failed: {}", e)
            );
            let error_msg = format!("{{\"error\":\"{}\"}}", e);
