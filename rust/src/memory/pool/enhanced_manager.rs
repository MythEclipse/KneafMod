...
pub struct EnhancedMemoryPoolManager {
    ...
    swap_pool: Option<std::sync::Arc<crate::memory::pool::swap::SwapMemoryPool>>,
    vec_pool: VecPool<u8>,
...
pub enum SmartPooledVec<T> {
    Hierarchical(crate::memory::pool::hierarchical::PooledVec<T>),
    /// Hierarchical buffer with an aligned view: (pooled_vec, offset_bytes, logical_len)
    HierarchicalAligned(crate::memory::pool::hierarchical::PooledVec<T>, usize, usize),
    Swap(crate::memory::pool::swap::SwapPooledVec),
    Vec(crate::memory::pool::specialized_pools::PooledVec<T>),
}
