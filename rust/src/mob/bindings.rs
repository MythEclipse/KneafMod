...
pub extern "system" fn Java_com_kneaf_core_performance_RustPerformance_processMobAiNative(
    ...
    jni_log_debug!(logger, &mut env, "JNI", "processMobAiNative called");
    ...
    jni_log_debug!(logger, &mut env, "JNI", &format!("Processing mob AI JSON with input length: {}", input_str.len()));
    match crate::mob::processing::process_mob_ai_json(&input_str) {
        Ok(result_json) => {
            jni_log_debug!(logger, &mut env, "JNI", &format!("process_mob_ai_json succeeded, result length: {}", result_json.len()));
        ...
        Err(e) => {
            jni_log_error!(logger, &mut env, "JNI", &format!("process_mob_ai_json failed: {}", e));
...
pub extern "system" fn Java_com_kneaf_core_performance_RustPerformance_processMobAiBinaryNative<'local>(
    ...
    let logger = get_logger();
    jni_log_debug!(logger, &mut env, "JNI", "processMobAiBinaryNative called");
    ...
    match process_mob_ai_binary_batch(slice) {
        Ok(result) => {
            jni_log_debug!(logger, &mut env, "JNI", &format!("process_mob_ai_binary_batch succeeded, result length: {}", result.len()));
        ...
        Err(e) => {
            jni_log_error!(logger, &mut env, "JNI", &format!("process_mob_ai_binary_batch failed: {}", e));
            let error_msg = format!("{{\"error\":\"{}\"}}", e);
