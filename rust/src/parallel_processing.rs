...
pub fn Java_com_kneaf_core_ParallelRustVectorProcessor_nalgebraMatrixMulDirect<'a>(
    ...
    if a_data.len() != 16 || b_data.len() != 16 {
        env.throw_new(
            ...
            "Buffers must contain 16 floats each",
        )
        .unwrap();
...
pub fn Java_com_kneaf_core_ParallelRustVectorProcessor_nalgebraVectorAddDirect<'a>(
    ...
    if a_data.len() != 3 || b_data.len() != 3 {
        env.throw_new(
            "java/lang/IllegalArgumentException",
            "Buffers must contain 3 floats each",
        )
        .unwrap();
        return result_buffer;
    }
...
pub fn copy_to_native_buffer(
    ...
    env.get_float_array_region(data, offset, &mut buffer)
        .unwrap();
    buffer
}
...
pub fn convert_jfloat_array_2d(
    env: &mut JNIEnv,
    arrays: &JObjectArray,
    count: i32,
) -> Vec<[f32; 16]> {
    ...
    for i in 0..count {
        ...
        env.get_float_array_region(&float_array, 0, &mut buffer)
            .unwrap();
...
pub fn create_jfloat_array_2d<'a>(env: &mut JNIEnv<'a>, data: Vec<[f32; 16]>) -> JObjectArray<'a> {
    ...
    let result_array = env
        .new_object_array(data.len() as i32, array_class, JObject::null())
        .unwrap();
    ...
    for (i, matrix) in data.iter().enumerate() {
        ...
        env.set_object_array_element(&result_array, i as i32, java_array)
            .unwrap();
    }
...
fn get_direct_buffer_data(env: &mut JNIEnv, buffer: JObject) -> Vec<f32> {
    let capacity = env
        .call_method(&buffer, "capacity", "()I", &[])
        .unwrap()
        .i()
        .unwrap() as usize;
    let address = env
        .call_method(&buffer, "address", "()J", &[])
        .unwrap()
        .j()
        .unwrap();
...
pub fn batch_nalgebra_matrix_mul(
    ...
    matrices_a
        ...
        .map(|(a, b)| nalgebra_matrix_mul(*a, *b))
        .collect()
}

pub fn batch_nalgebra_vector_add(
    vectors_a: Vec<[f32; 3]>,
    vectors_b: Vec<[f32; 3]>,
) -> Vec<[f32; 3]> {
    vectors_a
        ...
        .map(|(a, b)| nalgebra_vector_add(*a, *b))
        .collect()
}
...
pub fn Java_com_kneaf_core_RustNativeLoader_batchDistanceCalculationWithZeroCopy<'a>(
    ...
    let positions = MEMORY_MANAGER
        .retrieve_data(buffer_handle)
        .ok_or_else(|| {
            env.throw_new(
                "java/lang/IllegalArgumentException",
