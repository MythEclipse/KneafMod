...
async fn copy_dir_recursive_async(src: &Path, dst: &Path) -> Result<(), String> {
    ...
    let entries = tokio::fs::read_dir(src)
        .await
...
impl SwapMetadata {
    ...
    fn recalculate_priority(&mut self) {
        ...
        let swap_cost_score = if self.swap_count > 5 {
            -2.0
        } else if self.swap_count > 2 {
            -1.0
        } else {
            0.0
        };
...
fn try_read_lock_with_timeout<'a, T>(
    lock: &'a RwLock<T>,
    timeout_ms: u64,
    lock_name: &str,
) -> Result<RwLockReadGuard<'a, T>, DatabaseError> {
        ...
        match lock.try_read() {
            ...
            Err(_) => {
                if start_time.elapsed() >= timeout_duration {
                    error!(
                        "Timeout acquiring read lock for {} after {} ms",
                        lock_name, timeout_ms
                    );
...
fn try_write_lock_with_timeout<'a, T>(
    lock: &'a RwLock<T>,
    timeout_ms: u64,
    lock_name: &str,
) -> Result<RwLockWriteGuard<'a, T>, DatabaseError> {
    let start_time = std::time::Instant::now();
    let timeout_duration = Duration::from_millis(timeout_ms);
    ...
    loop {
        match lock.try_write() {
            ...
            Err(_) => {
                if start_time.elapsed() >= timeout_duration {
                    error!(
                        "Timeout acquiring write lock for {} after {} ms",
                        lock_name, timeout_ms
                    );
                    return Err(DatabaseError::TimeoutError(format!(
                        "Timeout acquiring write lock for {} after {} ms",
...
enum IoTask {
    Read {
        path: String,
        result_sender: oneshot::Sender<Result<Vec<u8>, DatabaseError>>,
    },
    Write {
        path: String,
        data: Vec<u8>,
        result_sender: oneshot::Sender<Result<usize, DatabaseError>>,
    },
    Delete {
        path: String,
        result_sender: oneshot::Sender<Result<(), DatabaseError>>,
    },
    AtomicWrite {
        temp_path: String,
        final_path: String,
...
impl CircuitBreaker {
    ...
    pub fn allow_operation(&self) -> bool {
        ...
        match state {
            ...
            2 => {
                // Half-open state - allow limited operations
                true
            }
            _ => false,
        }
    ...
    pub fn record_success(&self) {
        ...
        match state {
            1 => {
                // Just came from open state
                self.failure_count.store(0, Ordering::Relaxed);
                self.state.store(0, Ordering::Relaxed);
            }
            2 => {
                // Half-open state - success resets to closed
                self.failure_count.store(0, Ordering::Relaxed);
                self.state.store(0, Ordering::Relaxed);
            }
            _ => {
...
impl RustDatabaseAdapter {
    ...
    pub fn with_path(
        ...
        info!(
            "Initializing RustDatabaseAdapter of type: {} for world: {} at path: {}",
            database_type, world_name, db_path
        );
        ...
        let world_save_path = Path::new(db_path)
            .parent()
            .and_then(|p| p.parent())
            .and_then(|p| p.parent())
        ...
        info!(
            "Swap files will be stored in world save directory: {}",
            swap_path.display()
        );
        ...
        let db = if memory_mapping_enabled {
            ...
            sled::open(path).map_err(|e| format!("Failed to open sled database: {}", e))?
        };
    ...
    fn decompress_if_needed(&self, data: &[u8]) -> Result<Vec<u8>, String> {
        ...
        if is_compressed {
            ...
            let decompressed = block::decompress(payload, payload.len())
    ...
    pub fn put_chunk(&self, key: &str, data: &[u8]) -> Result<(), String> {
        ...
        // Update storage
        self.db
            .insert(key_bytes, data_to_store)
        ...
        // Update metadata and statistics with consistent lock ordering
                ...
                if exists {
                    ...
                    stats.total_size_bytes =
                        stats.total_size_bytes.saturating_sub(data_size / 2) + data_size;
                } else {
                    // New entry
    ...
    pub fn get_chunk(&self, key: &str) -> Result<Option<Vec<u8>>, String> {
        ...
        if let Ok(Some(_)) = processed_result {
            match acquire_locks_in_order(&self.stats, &self.swap_metadata, None, LOCK_TIMEOUT_MS) {
                ...
                Err(e) => {
                    warn!(
                        "Failed to acquire locks for get_chunk metadata update: {}",
                        e
                    );
            ...
            if let Ok(mut stats) =
                try_write_lock_with_timeout(&self.stats, LOCK_TIMEOUT_MS, "stats")
            {
                stats.read_latency_ms = start_time.elapsed().as_millis() as u64;
            }
        }
    ...
    pub fn perform_maintenance(&self) -> Result<(), String> {
        ...
        info!(
            "Database maintenance completed in {} ms",
            start_time.elapsed().as_millis()
        );
        Ok(())
    }
    ...
    pub fn create_backup(&self, backup_path: &str) -> Result<(), String> {
        ...
        std::fs::create_dir_all(&base).map_err(|e| {
            format!(
                "Failed to create base backup directory {}: {}",
                backup_path, e
            )
        })?;
        ...
        if db_path.exists() {
            let copy_result = if db_path.is_dir() {
                ...
                std::fs::copy(db_path, &dest).map(|_| ()).map_err(|e| {
                    format!("Failed to copy db file {:?} to {:?}: {}", db_path, dest, e)
                })
            };
        ...
        info!(
            "Created backup at {:?} ({} chunks, {} bytes)",
            backup_dir, stats.total_chunks, stats.total_size_bytes
        );
    ...
    pub fn verify_swap_chunk_checksum(&self, key: &str) -> Result<(), String> {
        ...
        let data = std::fs::read(&swap_file_path)
            .map_err(|e| format!("Failed to read swap file for checksum verification: {}", e))?;

        // Verify checksum using the same method as for regular chunks
        self.verify_and_extract_data(&data).map_err(|e| {
            format!(
                "Swap chunk checksum verification failed for key {}: {}",
                key, e
            )
        })?;
    ...
    pub fn swap_out_chunk(&self, key: &str) -> Result<(), String> {
        ...
        let data = self
            ...
            .map_err(|e| {
                format!(
                    "Failed to get chunk for swap out from world {}: {}",
                    self.world_name, e
                )
            })?
        ...
        let safe_key = key.replace(':', "_").replace('/', "-");
        let swap_file_path =
            Path::new(&self.swap_path).join(format!("{}_{}.swap", self.world_name, safe_key));
        let _swap_file_path_str = swap_file_path
            .to_str()
            .ok_or_else(|| "Failed to convert swap path to string".to_string())?;

        // Write data to swap file with atomic operation (temp file + rename) for safety
        let temp_path = Path::new(&self.swap_path).join(format!(
            "{}.tmp",
            swap_file_path
                .file_name()
                .unwrap_or_else(|| std::ffi::OsStr::new("swap_temp"))
                .to_string_lossy()
        ));
        std::fs::write(&temp_path, &data).map_err(|e| {
            format!(
                "Failed to write temp swap file for world {}: {}",
                self.world_name, e
            )
        })?;
        std::fs::rename(&temp_path, &swap_file_path).map_err(|e| {
            format!(
                "Failed to rename temp file for world {}: {}",
                self.world_name, e
            )
        })?;

        info!(
            "Created swap file for chunk {} at: {}",
            key,
            swap_file_path.display()
        );

        // Remove from main database
        self.db.remove(key_bytes).map_err(|e| {
            format!(
                "Failed to remove chunk from database for world {}: {}",
                self.world_name, e
            )
        })?;
        ...
        info!(
            "Swapped out chunk {} ({:.2}KB) from world {} to {}",
            key,
    ...
    pub fn swap_in_chunk(&self, key: &str) -> Result<Vec<u8>, String> {
        ...
        let matching_files: Vec<_> = std::fs::read_dir(swap_dir)
            .map_err(|e| {
                format!(
                    "Failed to read swap directory for world {}: {}",
                    self.world_name, e
                )
            })?
            .filter_map(|entry| {
                entry.ok().and_then(|e| {
                    ...
                    if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                        if filename.starts_with(&format!("{}_{}_", self.world_name, safe_key))
        ...
        let swap_file_path = matching_files
            .first()
            .ok_or_else(|| {
                format!(
                    "No swap files found for chunk {} in world {}",
                    key, self.world_name
                )
            })?
            .to_path_buf();
        ...
        let data = std::fs::read(&swap_file_path).map_err(|_e| {
            format!(
                "Failed to read swap file for world {}: {}",
                self.world_name,
                swap_file_path.display()
            )
        })?;
        ...
        let key_bytes = key.as_bytes();
        self.db.insert(key_bytes, data.clone()).map_err(|e| {
            format!(
                "Failed to restore chunk to database for world {}: {}",
                self.world_name, e
            )
        })?;
        ...
        for file_path in matching_files {
            std::fs::remove_file(&file_path).map_err(|_e| {
                format!(
                    "Failed to remove swap file for world {}: {}",
                    self.world_name,
                    file_path.display()
                )
            })?;
        ...
        info!(
            "Swapped in chunk {} ({:.2}KB) into world {} from {}",
            key,
            data_size as f64 / 1024.0,
            self.world_name,
            swap_file_path.display()
    ...
    pub fn get_swap_candidates(&self, limit: usize) -> Result<Vec<String>, String> {
        let swap_meta =
            try_read_lock_with_timeout(&self.swap_metadata, LOCK_TIMEOUT_MS, "swap_metadata")
    ...
    pub fn bulk_swap_out(&self, keys: &[String]) -> Result<usize, String> {
        ...
        for key in keys {
            ...
            let data_result = self.db.get(key_bytes).map_err(|e| {
                format!(
                    "Failed to get chunk for swap out in world {}: {}",
                    self.world_name, e
                )
            })?;
            ...
            let safe_key = key.replace(':', "_").replace('/', "-");
            let swap_file_path =
                format!("{}/{}_{}.swap", self.swap_path, self.world_name, safe_key);
        ...
        stats_guard.total_chunks = stats_guard
            .total_chunks
            .saturating_sub(success_count as u64);

        info!(
            "Bulk swap out completed for world {}: {}/{} chunks swapped successfully in {} ms",
            self.world_name,
            success_count,
            keys.len(),
            start_time.elapsed().as_millis()
    ...
    pub fn bulk_swap_in(&self, keys: &[String]) -> Result<Vec<Vec<u8>>, String> {
        ...
        match acquire_locks_in_order(&self.stats, &self.swap_metadata, None, LOCK_TIMEOUT_MS) {
                ...
                stats.swap_in_latency_ms = start_time.elapsed().as_millis() as u64;
                stats.total_swap_size_bytes =
                    stats.total_swap_size_bytes.saturating_sub(total_size);
        ...
        info!(
            "Bulk swap in completed: {}/{} chunks swapped successfully in {} ms",
            results.len(),
            keys.len(),
            start_time.elapsed().as_millis()
        );
    ...
    pub fn get_memory_mapped_chunk(&self, key: &str) -> Result<Option<Mmap>, String> {
        ...
        if let Ok(mm_files) = try_read_lock_with_timeout(
    ...
    pub fn store_chunk_raw_nbt(
        ...
        info!(
            "Stored chunk at ({}, {}) in dimension {} using FastNBT ({} bytes) in {} ms",
            x,
            z,
            dimension,
            serialized_data.len(),
    ...
    pub fn query_chunks_by_range(
        ...
        max_z: i32,
        dimension: &str,
    ) -> Result<Vec<ChunkCoordinates>, String> {
        let mut results = Vec::new();
    ...
    pub fn record_jni_operation_result(&self, operation_name: &str, success: bool) {
        if success {
            ...
            if error_count >= 5 {
                ...
                warn!(
                    "High error rate for {} operations: {} consecutive failures",
                    operation_name, error_count
                );
            }
        }
    }
    ...
    pub fn check_jni_operation_allowed(&self, operation_name: &str) -> Result<(), DatabaseError> {
        if !self.circuit_breaker.allow_operation() {
            return Err(DatabaseError::OperationFailed(format!(
                "{} operation failed: circuit breaker is open",
                operation_name
            )));
        }
        Ok(())
...
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeInit<'a>(
    ...
    let result = match RustDatabaseAdapter::new(
        &env.get_string(&database_type)
            .map_err(|e| DatabaseError::JniError(format!("Failed to get database type: {}", e)))
            .unwrap()
            .to_str()
            .unwrap()
            .to_string(),
        &env.get_string(&world_name)
            .map_err(|e| DatabaseError::JniError(format!("Failed to get world name: {}", e)))
            .unwrap()
            .to_str()
            .unwrap()
            .to_string(),
            ...
            match std::env::temp_dir().to_str() {
                Some(tmp) => {
                    let fallback_path = format!(
                        ...
                        tmp,
                        env.get_string(&database_type)
                            .map_err(|_| "Failed to get database type")
                            .unwrap()
                            .to_str()
                            .unwrap()
                    );
                    match RustDatabaseAdapter::with_path(
                        ...
                        &env.get_string(&database_type)
                            ...
                            .to_string(),
                        &env.get_string(&world_name)
                            .map_err(|_| "Failed to get world name")
                            .unwrap()
                            .to_str()
                            .unwrap()
                            .to_string(),
                        ...
                        Ok(fallback_adapter) => {
                            info!(
                                "Initialized fallback RustDatabaseAdapter at: {}",
                                fallback_path
                            );
                        ...
                        Err(e2) => {
                            ...
                            Err(DatabaseError::OperationFailed(format!(
                ...
                None => {
                    ...
                    Err(DatabaseError::OperationFailed(
                        "Failed to determine temp directory".to_string(),
                    ))
                }
            }
        }
    ...
    match result {
        ...
        Err(e) => {
            ...
            if let Ok(adapter) = env
                .get_string(&database_type)
                .map(|s| s.to_str().unwrap().to_string())
            {
                let _dummy_adapter = RustDatabaseAdapter::new(
                    &adapter,
                    &env.get_string(&world_name)
                        .map(|s| s.to_str().unwrap().to_string())
                        .unwrap_or_default(),
            ...
            env.throw_new(exception_class, &e.to_string()).unwrap();
            0
        }
    }
}
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeDeleteChunk<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeGetChunkCount<
    'a,
>(
    _env: JNIEnv<'a>,
    _class: JClass<'a>,
...
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeGetStats<'a>(
    ...
    match adapter.get_stats() {
        Ok(stats) => {
            ...
            let stats_class = env
                .find_class("com/kneaf/core/chunkstorage/DatabaseAdapter$DatabaseStats")
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativePerformMaintenance<
    'a,
>(
    _env: JNIEnv<'a>,
    _class: JClass<'a>,
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeCreateBackup<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeGetDatabaseType<
    'a,
>(
    env: JNIEnv<'a>,
    _class: JClass<'a>,
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeSwapOutChunk<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeSwapInChunk<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeGetSwapCandidates<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
    ...
    match adapter.get_swap_candidates(limit as usize) {
        Ok(candidates) => {
            ...
            for candidate in candidates {
                let candidate_str = env.new_string(&candidate).expect("Failed to create string");
                let candidate_obj = JObject::from(candidate_str);
                env.call_method(
                    &list_obj,
                    "add",
                    "(Ljava/lang/Object;)Z",
                    &[jni::objects::JValue::Object(&candidate_obj)],
                )
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeBulkSwapOut<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeStoreChunkRawNbt<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
    ...
    let nbt_data_vec = env
        .convert_byte_array(&raw_nbt_data)
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeGetChunkParsedNbt<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeQueryChunksByRange<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
    ...
    match adapter.query_chunks_by_range(
        min_x as i32,
        max_x as i32,
        min_z as i32,
        max_z as i32,
        ...
        Ok(chunks) => {
            ...
            let coord_class = env
                .find_class("com/kneaf/core/chunkstorage/ChunkCoordinates")
                    ...
                    .new_object(
                        ...
                        &[
                            ...
                            jni::objects::JValue::Object(
                                &env.new_string(&chunk.dimension)
...
#[no_mangle]
pub extern "system" fn Java_com_kneaf_core_chunkstorage_RustDatabaseAdapter_nativeGetChunkStatsByDimension<
    'a,
>(
    mut env: JNIEnv<'a>,
    _class: JClass<'a>,
    ...
    match adapter.get_chunk_stats_by_dimension(&dimension_str) {
        Ok((chunk_count, total_size)) => {
            ...
            let stats_class = env
                .find_class("com/kneaf/core/chunkstorage/ChunkStats")
