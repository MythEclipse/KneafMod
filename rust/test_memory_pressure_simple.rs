//! Simple test for memory pressure configuration
...
lazy_static! {
    static ref GLOBAL_MEMORY_PRESSURE_CONFIG: Arc<RwLock<MemoryPressureConfig>> = {
        let config = MemoryPressureConfig::new(0.70, 0.85, 0.95, 0.98)
            .expect("Failed to create default memory pressure config");
