...
public class AsyncMonitorCommands {
        ...
        private static int showAsyncStats(CommandContext<CommandSourceStack> context) {
                ...
                source.sendSuccess(() -> Component.literal(String.format("§eInitialized: §f%s", stats.initialized)),
                                false);
                ...
                source.sendSuccess(
                                () -> Component.literal(String.format("  §eActive loggers: §f%d",
                                                stats.loggerStats.activeLoggers)),
                                false);
                source.sendSuccess(
                                () -> Component
                                                .literal(String.format("  §eMessages logged: §f%d",
                                                                stats.loggerStats.totalMessagesLogged)),
                                false);
                source.sendSuccess(
                                () -> Component
                                                .literal(String.format("  §eMessages dropped: §f%d",
                                                                stats.loggerStats.totalMessagesDropped)),
                                false);
                source.sendSuccess(
                                () -> Component
                                                .literal(String.format("  §ePending messages: §f%d",
                                                                stats.loggerStats.totalPendingMessages)),
                                false);

                source.sendSuccess(() -> Component.literal("§6\n[Metrics]"), false);
                source.sendSuccess(
                                () -> Component.literal(
                                                String.format("  §eCounters: §f%d", stats.metricsStats.counterCount)),
                                false);
                source.sendSuccess(
                                () -> Component.literal(
                                                String.format("  §eGauges: §f%d", stats.metricsStats.gaugeCount)),
                                false);
                source.sendSuccess(
                                () -> Component.literal(String.format("  §eHistograms: §f%d",
                                                stats.metricsStats.histogramCount)),
                                false);
                source.sendSuccess(
                                () -> Component.literal(
                                                String.format("  §eTimers: §f%d", stats.metricsStats.timerCount)),
                                false);
                source.sendSuccess(
                                () -> Component.literal(String.format("  §eTotal recorded: §f%d",
                                                stats.metricsStats.totalRecorded)),
                                false);
                source.sendSuccess(
                                () -> Component.literal(
                                                String.format("  §eDropped: §f%d", stats.metricsStats.totalDropped)),
                                false);

                source.sendSuccess(() -> Component.literal("§6\n[Async Operations]"), false);
                source.sendSuccess(
                                () -> Component.literal(
                                                String.format("  §eTotal async: §f%d", stats.totalAsyncOperations)),
                                false);
                source.sendSuccess(
                                () -> Component.literal(
                                                String.format("  §eSync fallbacks: §f%d", stats.totalSyncFallbacks)),
                                false);
                source.sendSuccess(
                                () -> Component.literal(String.format("  §eSuccess rate: §f%.2f%%",
                                                stats.getAsyncSuccessRate() * 100)),
                                false);
