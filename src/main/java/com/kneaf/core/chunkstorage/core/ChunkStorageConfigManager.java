...
/**
 * Manages configuration for chunk storage operations.
 * Handles eviction policy creation and configuration validation.
 */
public class ChunkStorageConfigManager {
    ...
    public ChunkCache.EvictionPolicy createEvictionPolicy() {
        ...
        switch (evictionPolicy.toLowerCase()) {
            ...
            default:
                ChunkStorageExceptionHandler.logWarning("ChunkStorageConfigManager", 
                    "Unknown eviction policy '{}', defaulting to LRU", evictionPolicy);
                return new ChunkCache.LRUEvictionPolicy();
        }
    }
    ...
    public SwapManager.SwapConfig createSwapConfig() {
        SwapManager.SwapConfig swapConfig =
            new SwapManager.SwapConfig();
    ...
    public java.util.concurrent.ThreadFactory createThreadFactory(String worldName) {
        return new java.util.concurrent.ThreadFactory() {
            ...
            public Thread newThread(Runnable r) {
                Thread thread = new Thread(r, ChunkStorageConstants.THREAD_NAME_PREFIX_CHUNK_STORAGE + worldName + "-" + threadNumber.getAndIncrement());
    ...
    public String getConfigurationSummary() {
        return String.format("ChunkStorageConfig{enabled=%s, cacheCapacity=%d, evictionPolicy='%s', " +
                           ...
                           config.isEnabled(), config.getCacheCapacity(), config.getEvictionPolicy(),
                           config.getAsyncThreadPoolSize(), config.getDatabaseType(), config.isUseRustDatabase(),
                           config.isEnableSwapManager(), config.getMaxConcurrentSwaps(), config.getSwapBatchSize());
    }
}