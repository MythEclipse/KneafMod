...
/**
 * Coordinates between different chunk storage components.
 * Handles swap operations, memory pressure management, and component integration.
 */
public class ChunkStorageCoordinator implements StorageStatisticsProvider {
     ...
     */
    public CompletableFuture<Integer> bulkSwapChunks(java.util.List<String> chunkKeys, Object operationType) {
        if (!enabled || swapManager == null) {
    ...
    public ChunkStorageManager.StorageStats getStorageStats() {
        ...
        try {
            ...
            if (swapStatsObj != null) {
                try {
                    swapEnabled = (boolean) swapStatsObj.getClass().getMethod("isEnabled").invoke(swapStatsObj);
                    Object pressureLevelObj = swapStatsObj.getClass().getMethod("getPressureLevel").invoke(swapStatsObj);
                    memoryPressureLevel = pressureLevelObj.toString();
                    swapOperationsTotal = (long) swapStatsObj.getClass().getMethod("getTotalOperations").invoke(swapStatsObj);
                    swapOperationsFailed = (long) swapStatsObj.getClass().getMethod("getFailedOperations").invoke(swapStatsObj);
                } catch (Exception e) {
    ...
    private Object createEmptyDatabaseStats() {
        ...
        return new Object() {
            public long getTotalChunks() { return 0; }
            public long getReadLatencyMs() { return 0; }
            public long getWriteLatencyMs() { return 0; }
