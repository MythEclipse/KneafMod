...
/**
 * Facade for chunk storage operations that coordinates between core components.
 * This class provides a simplified interface to the complex chunk storage subsystem.
 */
public class ChunkStorageManager implements StorageStatisticsProvider {
    ...
    public ChunkStorageManager(String worldName, ChunkStorageConfig config) {
        ...
        if (enabled) {
            ...
            this.coordinator = swapManager != null ? new ChunkStorageCoordinator(worldName, core, swapManager, config.isEnabled()) : null;
            
            LOGGER.info("Initialized ChunkStorageManager facade for world '{}' with config: {}", 
                       worldName, configManager.getConfigurationSummary());
        } else {
    ...
    public Optional<Object> loadChunk(Object level, int chunkX, int chunkZ) {
        ...
        try {
            ...
            LOGGER.error("ChunkStorageManager: Failed to load chunk at (" + chunkX + ", " + chunkZ + ")", e);
            return Optional.empty();
        }
    }
    ...
    public CompletableFuture<Optional<Object>> loadChunkAsync(Object level, int chunkX, int chunkZ) {
        ...
        try {
            return CompletableFuture.supplyAsync(() -> core.loadChunk(level, chunkX, chunkZ),
                java.util.concurrent.Executors.newSingleThreadExecutor());
        } catch (Exception e) {
            LOGGER.error("ChunkStorageManager: Failed to load chunk async at (" + chunkX + ", " + chunkZ + ")", e);
            return CompletableFuture.completedFuture(Optional.empty());
        }
    }
    ...
    public boolean deleteChunk(Object level, int chunkX, int chunkZ) {
        ...
        try {
            ...
            LOGGER.error("ChunkStorageManager: Failed to delete chunk at (" + chunkX + ", " + chunkZ + ")", e);
            return false;
        }
    }
    ...
    public void performMaintenance() {
        ...
        try {
            ...
            LOGGER.error("ChunkStorageManager: Failed to perform storage maintenance for world '" + worldName + "'", e);
        }
    }
    ...
    public void shutdown() {
        ...
        try {
            ...
            LOGGER.error("ChunkStorageManager: Error during shutdown of ChunkStorageManager facade for world '" + worldName + "'", e);
        }
    }
    ...
    public void createBackup(String backupPath) {
        ...
        try {
            ...
            LOGGER.error("ChunkStorageManager: Failed to create backup for world '" + worldName + "' at '" + backupPath + "'", e);
        }
    }
    ...
    public CompletableFuture<Boolean> swapOutChunk(String chunkKey) {
        ...
        try {
            return coordinator != null ? coordinator.swapOutChunk(chunkKey) :
    ...
    public CompletableFuture<Boolean> swapInChunk(String chunkKey) {
        ...
        try {
            return coordinator != null ? coordinator.swapInChunk(chunkKey) :
                CompletableFuture.completedFuture(false);
        } catch (Exception e) {
    ...
    public Object getMemoryPressureLevel() {
        ...
        try {
            return coordinator != null ? coordinator.getMemoryPressureLevel() :
    ...
    private DatabaseAdapter createDatabaseAdapter(String worldName, ChunkStorageConfig config) {
        if (config.isUseRustDatabase()) {
            try {
                LOGGER.info("Attempting to create RustDatabaseAdapter with type: {}, checksums: {}",
                           config.getDatabaseType(), config.isEnableChecksums());
