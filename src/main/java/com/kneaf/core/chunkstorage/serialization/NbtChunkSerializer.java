...
/**
 * NBT-based implementation of ChunkSerializer. Uses Minecraft's built-in NBT format for chunk
 * serialization. This implementation includes proper error handling for test environments.
 */
public class NbtChunkSerializer implements ChunkSerializer {
  ...
  /**
   * Actual serialization implementation that uses Minecraft classes. This method is only called if
   * Minecraft classes are available.
   */
  private byte[] serializeChunk(Object chunk) throws IOException {
    try {
      ...
      compoundTagClass
          ...
          .invoke(rootTag, TAG_VERSION, FORMAT_VERSION);
      compoundTagClass
          .getMethod("putLong", String.class, long.class)
          .invoke(rootTag, TAG_TIMESTAMP, System.currentTimeMillis());
      ...
      compoundTagClass
          .getMethod("putInt", String.class, int.class)
          .invoke(chunkData, "xPos", chunkX);
      compoundTagClass
          .getMethod("putInt", String.class, int.class)
          .invoke(chunkData, "zPos", chunkZ);

      try {
        Object fullStatus = levelChunkClass.getMethod("getFullStatus").invoke(chunk);
        String statusName = fullStatus.toString();
        compoundTagClass
            .getMethod("putString", String.class, String.class)
            .invoke(chunkData, "status", statusName);
      } catch (Exception e) {
        LOGGER.debug("Could not get chunk status, using default");
        compoundTagClass
            .getMethod("putString", String.class, String.class)
            .invoke(chunkData, "status", "FULL");
      }

      // Save section data for all sections
      serializeChunkSections(
          chunk, chunkData, levelChunkClass, compoundTagClass, listTagClass, blockStateClass);

      // Save heightmaps
      serializeHeightmaps(
          chunk, chunkData, levelChunkClass, compoundTagClass, blockPosClass, blockStateClass);

      // Save block entities
      serializeBlockEntities(
          chunk, chunkData, levelChunkClass, compoundTagClass, blockPosClass, blockEntityClass);

      compoundTagClass
          .getMethod("put", String.class, compoundTagClass)
          .invoke(rootTag, TAG_CHUNK_DATA, chunkData);
      ...
      compoundTagClass
          .getMethod("putLong", String.class, long.class)
          .invoke(rootTag, TAG_CHECKSUM, checksum);
  ...
  /**
   * Actual deserialization implementation that uses Minecraft classes. This method is only called
   * if Minecraft classes are available.
   */
  private Object deserializeChunk(byte[] data) throws IOException {
    try {
      ...
      int version =
          (int) compoundTagClass.getMethod("getInt", String.class).invoke(rootTag, TAG_VERSION);
      ...
      if ((boolean)
          compoundTagClass
              ...
              .invoke(rootTag, TAG_CHECKSUM, 4)) { // 4 = Long tag type
        Object chunkData =
            compoundTagClass.getMethod("getCompound", String.class).invoke(rootTag, TAG_CHUNK_DATA);
        byte[] chunkBytes = writeCompoundTagToBytes(chunkData, nbtIoClass);
        long expectedChecksum =
            (long)
                compoundTagClass.getMethod("getLong", String.class).invoke(rootTag, TAG_CHECKSUM);
  ...
  private void serializeChunkSections(
      Object chunk,
      Object chunkData,
      Class<?> levelChunkClass,
    ...
    for (int y = minSection; y < maxSection; y++) {
      Object section =
          levelChunkClass
              .getMethod("getSection", int.class)
              .invoke(
                  chunk,
                  levelChunkClass
                      .getMethod("getSectionIndexFromSectionY", int.class)
                      .invoke(chunk, y));

      if (section != null
          && !(boolean) section.getClass().getMethod("hasOnlyAir").invoke(section)) {
    ...
    compoundTagClass
        .getMethod("put", String.class, listTagClass)
        .invoke(chunkData, "sections", sectionsList);
  }

  /** Serialize block states for a chunk section. */
  private void serializeBlockStates(
      Object section,
      Object sectionTag,
      Class<?> compoundTagClass,
      Class<?> listTagClass,
      ...
      for (int y = 0; y < 16; y++) {
        for (int z = 0; z < 16; z++) {
          ...
          if (!(boolean) blockStateClass.getMethod("isAir").invoke(blockState)) {
            ...
            compoundTagClass
                .getMethod("putString", String.class, String.class)
                .invoke(blockTag, "block", blockState.toString());
    ...
    compoundTagClass
        .getMethod("put", String.class, listTagClass)
        .invoke(sectionTag, "block_states", blockStatesList);
  }
  ...
  private void serializeHeightmaps(
    ...
    try {
      ...
      compoundTagClass
          .getMethod("putInt", String.class, int.class)
          .invoke(heightmapsTag, "minHeight", minHeight);
      compoundTagClass
          .getMethod("putInt", String.class, int.class)
          .invoke(heightmapsTag, "maxHeight", maxHeight);
      ...
      for (int x = 0; x < 16; x += 4) {
        for (int z = 0; z < 16; z += 4) {
          ...
          compoundTagClass
              .getMethod("putInt", String.class, int.class)
              .invoke(heightmapsTag, key, surfaceY);
        }
      }
    } catch (Exception e) {
    ...
    compoundTagClass
        .getMethod("put", String.class, compoundTagClass)
        .invoke(chunkData, "heightmaps", heightmapsTag);
  }

  /** Serialize block entities from a chunk. */
  private void serializeBlockEntities(
      Object chunk,
      Object chunkData,
      Class<?> levelChunkClass,
      Class<?> compoundTagClass,
    ...
    if (blockEntitiesPos instanceof Iterable) {
      for (Object pos : (Iterable<?>) blockEntitiesPos) {
        Object blockEntity =
            levelChunkClass.getMethod("getBlockEntity", blockPosClass).invoke(chunk, pos);
        if (blockEntity != null) {
          try {
            ...
            try {
              ...
              compoundTagClass
                  .getMethod("putString", String.class, String.class)
                  .invoke(beTag, "id", typeString);
            } catch (Exception e) {
              compoundTagClass
                  .getMethod("putString", String.class, String.class)
                  .invoke(beTag, "id", "unknown");
            }
  ...
  private int findSurfaceHeight(
    ...
    for (int y = maxHeight - 1; y >= minHeight; y--) {
      ...
      Object blockState =
          levelChunkClass.getMethod("getBlockState", blockPosClass).invoke(chunk, pos);
      if (!(boolean) blockStateClass.getMethod("isAir").invoke(blockState)) {
        return y;
      }
    }
  ...
  private byte[] writeCompoundTagToBytes(Object tag, Class<?> nbtIoClass) throws Exception {
    ...
    try {
      nbtIoClass
          .getMethod(
              "write", Class.forName("net.minecraft.nbt.CompoundTag"), java.io.DataOutput.class)
          .invoke(null, tag, dos);
