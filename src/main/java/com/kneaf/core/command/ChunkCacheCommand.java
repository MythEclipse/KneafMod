...
/**
 * Command for monitoring and managing chunk cache operations.
 * Provides statistics, configuration, and maintenance operations.
 */
public class ChunkCacheCommand {
    private static final Logger LOGGER = LogUtils.getLogger();
    
    // Storage managers per world
    private static final Map<String, ChunkStorageManager> storageManagers = new ConcurrentHashMap<>();
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal("chunkcache")
            .requires(source -> source.hasPermission(2)) // Requires OP level 2
            .then(Commands.literal("stats")
                .executes(ChunkCacheCommand::showStats))
            .then(Commands.literal("clear")
                .executes(ChunkCacheCommand::clearCache))
            .then(Commands.literal("maintenance")
                .executes(ChunkCacheCommand::performMaintenance))
            .then(Commands.literal("backup")
                .executes(ChunkCacheCommand::createBackup))
            .then(Commands.literal("config")
                .then(Commands.literal("capacity")
                    .then(Commands.argument("size", IntegerArgumentType.integer(1, 10000))
                        .executes(context -> setCacheCapacity(context, IntegerArgumentType.getInteger(context, "size")))))
                .then(Commands.literal(ARG_POLICY)
                    .then(Commands.argument(ARG_POLICY, StringArgumentType.string())
                        .executes(context -> setEvictionPolicy(context, StringArgumentType.getString(context, ARG_POLICY))))))
            .then(Commands.literal("help")
                .executes(ChunkCacheCommand::showHelp));
    ...
    public static void registerStorageManager(String worldName, ChunkStorageManager manager) {
        if (worldName != null && manager != null) {
            storageManagers.put(worldName, manager);
    ...
    public static void unregisterStorageManager(String worldName) {
        if (worldName != null) {
            ChunkStorageManager removed = storageManagers.remove(worldName);
            if (removed != null) {
                LOGGER.info("Unregistered ChunkStorageManager for world '{}' from command system", worldName);
            }
        }
    }
    ...
    private static int showStats(CommandContext<CommandSourceStack> context) {
        ...
        for (Map.Entry<String, ChunkStorageManager> entry : storageManagers.entrySet()) {
            ...
            if (stats.isEnabled()) {
                ...
                source.sendSuccess(() -> Component.literal(
                    String.format(pattern,
                                worldName, stats.getStatus(), stats.getTotalChunksInDb(),
                                stats.getCachedChunks(), stats.getAvgReadLatencyMs(),
                                stats.getAvgWriteLatencyMs(), stats.getCacheHitRate() * 100,
                                stats.getOverallHitRate() * 100)
                ), false);
            } else {
                source.sendSuccess(() -> Component.literal(
                    String.format("§bWorld: §f%s §7[DISABLED]", worldName)
                ), false);
            }
        }
    ...
    private static int clearCache(CommandContext<CommandSourceStack> context) {
        ...
        int clearedCount = storageManagers.size();
        ...
        source.sendSuccess(() -> Component.literal(
            String.format("§aCleared caches for §f%d §aworlds", clearedCount)
        ), false);
    ...
    private static int performMaintenance(CommandContext<CommandSourceStack> context) {
        ...
        for (Map.Entry<String, ChunkStorageManager> entry : storageManagers.entrySet()) {
            ...
            try {
                ...
                source.sendSuccess(() -> Component.literal(
                    String.format("§cMaintenance failed for world '%s': %s", worldName, e.getMessage())
                ), false);
    ...
    private static int createBackup(CommandContext<CommandSourceStack> context) {
        ...
        for (Map.Entry<String, ChunkStorageManager> entry : storageManagers.entrySet()) {
            ...
            try {
                ...
                backupCount++;
                source.sendSuccess(() -> Component.literal(
                    String.format("§aBackup created for world '%s' at '%s'", worldName, backupPath)
                ), false);
            } catch (Exception e) {
                source.sendSuccess(() -> Component.literal(
    ...
    private static int setCacheCapacity(CommandContext<CommandSourceStack> context, int capacity) {
        ...
        // Note: In a real implementation, we'd need to add a setCacheCapacity method to ChunkStorageManager
    ...
    private static int setEvictionPolicy(CommandContext<CommandSourceStack> context, String policy) {
        ...
        // Validate policy
        if (!policy.equalsIgnoreCase("LRU") && !policy.equalsIgnoreCase("Distance") && 
            !policy.equalsIgnoreCase("Hybrid")) {
            source.sendSuccess(() -> Component.literal(
                String.format("§cInvalid eviction policy '%s'. Valid policies: LRU, Distance, Hybrid", policy)
            ), false);
            return 0;
        }
        
        // Note: In a real implementation, we'd need to add a setEvictionPolicy method to ChunkStorageManager
        // For now, we'll just acknowledge the command
    ...
    private static int showHelp(CommandContext<CommandSourceStack> context) {
        ...
        source.sendSuccess(() -> Component.literal("§b/chunkcache stats §7- Show storage statistics"), false);
        source.sendSuccess(() -> Component.literal("§b/chunkcache clear §7- Clear all caches"), false);
        source.sendSuccess(() -> Component.literal("§b/chunkcache maintenance §7- Perform storage maintenance"), false);
        source.sendSuccess(() -> Component.literal("§b/chunkcache backup §7- Create backups"), false);
        source.sendSuccess(() -> Component.literal("§b/chunkcache config capacity <size> §7- Set cache capacity"), false);
    source.sendSuccess(() -> Component.literal(String.format("§b/chunkcache config %s <LRU|Distance|Hybrid> §7- Set eviction policy", ARG_POLICY)), false);
