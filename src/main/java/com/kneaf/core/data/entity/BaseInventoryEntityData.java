...
public abstract class BaseInventoryEntityData extends BaseEntityData {
   ...
   */
  protected BaseInventoryEntityData(
      long id,
      double x,
      double y,
      double z,
  ...
  public boolean validate() {
    ...
    if (inventorySize < 0) {
      throw new DataValidationException(
          "inventorySize", inventorySize, "Inventory size must not be negative");
    }
    ...
    if (inventoryItems.size() > inventorySize) {
      throw new DataValidationException(
          ...
          inventoryItems.size(),
          String.format(
              "Item count (%d) cannot exceed inventory size (%d)",
              inventoryItems.size(), inventorySize));
    }
