...
/**
 * Legacy compatibility wrapper for DatabaseException. Extends the new enhanced exception class to
...
public class DatabaseException extends RuntimeException {
  ...
  public DatabaseException(DatabaseErrorType errorType, String message) {
    super(message);
    this.delegate =
        com.kneaf.core.exceptions.storage.DatabaseException.builder()
  ...
  public DatabaseException(
      DatabaseErrorType errorType, String databaseType, String message, Throwable cause) {
    super(message, cause);
    this.delegate =
        com.kneaf.core.exceptions.storage.DatabaseException.builder()
            .errorType(convertErrorType(errorType))
  ...
  public static DatabaseException asyncOperationFailed(
      String operation, String databaseType, String key, Throwable cause) {
    return new DatabaseException(
        DatabaseErrorType.ASYNC_OPERATION_FAILED,
        operation,
        databaseType,
        key,
        String.format("Async %s operation failed", operation),
        cause);
  }

  /** Creates a database exception for native library errors. */
  public static DatabaseException nativeLibraryError(
      String databaseType, String message, Throwable cause) {
    return new DatabaseException(
        DatabaseErrorType.NATIVE_LIBRARY_ERROR, databaseType, message, cause);
  }

  private static com.kneaf.core.exceptions.storage.DatabaseException.DatabaseErrorType
      convertErrorType(DatabaseErrorType errorType) {
    if (errorType == null) return null;

    switch (errorType) {
      case CONNECTION_FAILED:
        return com.kneaf.core.exceptions.storage.DatabaseException.DatabaseErrorType
            .CONNECTION_FAILED;
  ...
  private static DatabaseErrorType convertErrorTypeBack(
      com.kneaf.core.exceptions.storage.DatabaseException.DatabaseErrorType errorType) {
    if (errorType == null) return null;
