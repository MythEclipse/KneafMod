...
/**
 * Legacy compatibility wrapper for NativeLibraryException. Extends the new enhanced exception class
...
public class NativeLibraryException extends RuntimeException {
  ...
  public NativeLibraryException(NativeErrorType errorType, String message) {
    super(message);
    this.delegate =
        com.kneaf.core.exceptions.nativelib.NativeLibraryException.builder()
  ...
  public static NativeLibraryException libraryNotAvailable(String libraryName) {
    return new NativeLibraryException(
        NativeErrorType.LIBRARY_NOT_AVAILABLE,
        libraryName,
        null,
        String.format("Native library '%s' is not available", libraryName),
        null);
  }

  /** Creates a native library exception for binary protocol errors. */
  public static NativeLibraryException binaryProtocolError(
      String operation, String details, Throwable cause) {
    return new NativeLibraryException(
        NativeErrorType.BINARY_PROTOCOL_ERROR,
        "rustperf",
        operation,
        String.format("Binary protocol error in %s: %s", operation, details),
        cause);
  }

  private static com.kneaf.core.exceptions.nativelib.NativeLibraryException.NativeErrorType
      convertErrorType(NativeErrorType errorType) {
    if (errorType == null) return null;

    switch (errorType) {
      case LIBRARY_NOT_AVAILABLE:
        return com.kneaf.core.exceptions.nativelib.NativeLibraryException.NativeErrorType
  ...
  private static NativeErrorType convertErrorTypeBack(
      com.kneaf.core.exceptions.nativelib.NativeLibraryException.NativeErrorType errorType) {
    if (errorType == null) return null;
