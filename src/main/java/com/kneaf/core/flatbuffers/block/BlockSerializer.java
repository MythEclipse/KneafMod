...
/**
 * Serializer for block entity data using flatbuffer format.
 * Handles serialization and deserialization of block entity processing results.
 */
public class BlockSerializer extends BaseFlatBufferSerializer<BlockInput, List<Long>> {
    ...
    public BlockSerializer(BufferPool bufferPool) {
        super(SerializationConstants.ENTITY_TYPE_BLOCK, SerializationConstants.SCHEMA_VERSION_1_0, bufferPool);
    }
    ...
    public BlockSerializer(BufferPool bufferPool, SchemaValidator<BlockInput> validator) {
        super(SerializationConstants.ENTITY_TYPE_BLOCK, SerializationConstants.SCHEMA_VERSION_1_0, bufferPool, validator);
    }
    
    @Override
    protected void serializeToBufferInternal(BlockInput input, ByteBuffer buffer) throws SerializationException {
        if (input == null) {
            throw new SerializationException("Block input cannot be null", getSerializerType(), "serializeToBufferInternal");
        }
        ...
        try {
            ...
            throw new SerializationException("Failed to serialize block input", e, getSerializerType(), 
    ...
    protected List<Long> deserializeFromBufferInternal(ByteBuffer buffer) throws SerializationException {
        try {
            ...
            if (blockEntityCount < 0) {
                throw new SerializationException("Invalid block entity count: " + blockEntityCount, getSerializerType(), 
            ...
            throw new SerializationException("Failed to deserialize block result", e, getSerializerType(), 
