...
/**
 * Utility methods for flatbuffer serialization operations. Provides helper methods for buffer
 * management, data conversion, and validation.
 */
public final class SerializationUtils {
  ...
  /**
   * Create a new ByteBuffer with the specified initial capacity. The buffer will be configured for
  ...
  /**
   * Create a new ByteBuffer from existing byte array. The buffer will be configured for
   * little-endian byte order.
   *
  ...
  /**
   * Ensure buffer has enough remaining capacity for the required bytes. If not enough space,
   * creates a new larger buffer and copies existing data.
   *
  ...
  public static ByteBuffer ensureCapacity(ByteBuffer buffer, int requiredBytes) {
    ...
    int newCapacity =
        Math.max(
            currentCapacity * SerializationConstants.BUFFER_GROWTH_FACTOR,
  ...
  /**
   * Write a string to the buffer with length prefix. Format: [length:4 bytes][string bytes]
   *
  ...
  /**
   * Read a string from the buffer with length prefix. Format: [length:4 bytes][string bytes]
   *
