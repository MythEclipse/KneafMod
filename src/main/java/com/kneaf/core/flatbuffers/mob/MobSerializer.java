...
/**
 * Serializer for mob data using flatbuffer format.
 * Handles serialization and deserialization of mob AI processing results.
 */
public class MobSerializer extends BaseFlatBufferSerializer<MobInput, MobProcessResult> {
    ...
    public MobSerializer(BufferPool bufferPool) {
        super(SerializationConstants.ENTITY_TYPE_MOB, SerializationConstants.SCHEMA_VERSION_1_0, bufferPool);
    }
    ...
    public MobSerializer(BufferPool bufferPool, SchemaValidator<MobInput> validator) {
        super(SerializationConstants.ENTITY_TYPE_MOB, SerializationConstants.SCHEMA_VERSION_1_0, bufferPool, validator);
    }
    
    @Override
    protected void serializeToBufferInternal(MobInput input, ByteBuffer buffer) throws SerializationException {
        if (input == null) {
            throw new SerializationException("Mob input cannot be null", getSerializerType(), "serializeToBufferInternal");
        }
        ...
        try {
            ...
            throw new SerializationException("Failed to serialize mob input", e, getSerializerType(), 
    ...
    protected MobProcessResult deserializeFromBufferInternal(ByteBuffer buffer) throws SerializationException {
        try {
            ...
            if (disableCount < 0) {
                throw new SerializationException("Invalid disable count: " + disableCount, getSerializerType(), 
            ...
            if (simplifyCount < 0) {
                throw new SerializationException("Invalid simplify count: " + simplifyCount, getSerializerType(), 
                                               "deserializeFromBufferInternal");
            }
            ...
            throw new SerializationException("Failed to deserialize mob result", e, getSerializerType(), 
