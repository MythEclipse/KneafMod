 ...
 *
 * This class intentionally avoids any hard dependency on loader-specific
 * networking libraries (Forge/Fabric). It uses vanilla packet classes which
 * are available to both client and server sides in this environment. The
 * client-side receiver still needs to be implemented to fully render the
 * overlay.
 */
public final class NetworkHandler {
    ...
    public static final ResourceLocation PERFORMANCE_CHANNEL = ResourceLocation.parse(KneafCore.MODID + ":perf_overlay");
    ...
    /**
     * Send a single text line to a player using a vanilla custom payload.
     * The payload format is a single UTF-8 string written via FriendlyByteBuf.writeUtf.
     */
    public static void sendPerformanceLineToPlayer(ServerPlayer player, String line) {
        ...
        try {
            ...
            try {
                java.lang.reflect.Constructor<?> ctor = pktClass.getConstructor(ResourceLocation.class, FriendlyByteBuf.class);
                ...
                try {
                    java.lang.reflect.Constructor<?> ctor2 = pktClass.getConstructor(String.class, FriendlyByteBuf.class);
                    ...
                    LOGGER.debug("Packet constructor mismatch for ClientboundCustomPayloadPacket: {}", ex2.getMessage());
                }
            }
        } catch (ClassNotFoundException cnfe) {
            // Packet class not available in this mapping; fall through to chat fallback
        } catch (Exception e) {
            LOGGER.debug("Reflection send error when sending performance payload to {}: {}", player.getGameProfile().getName(), e.getMessage());
        }

        // Fallback: use chat/system message so the client receives the line even if custom payload isn't available
        try {
            player.displayClientMessage(net.minecraft.network.chat.Component.literal(line), false);
        } catch (Exception ex) {
            LOGGER.debug("Failed to fallback-send performance line to {}: {}", player.getGameProfile().getName(), ex.getMessage());
        }
    }
