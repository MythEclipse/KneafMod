...
/**
 * Legacy RustPerformance class - now acts as a facade to the new refactored system.
 * This maintains backward compatibility while delegating to the new RustPerformanceFacade.
 */
public class RustPerformance {
    
    private static final RustPerformanceFacade facade = RustPerformanceFacade.getInstance();
    private static final PerformanceMonitor simpleMonitor = new PerformanceMonitor() {
        @Override
        ...
        @Override
        public long getTotalEntitiesProcessed() { return 0; }
        @Override
        public long getTotalItemsProcessed() { return 0; }
        @Override
        public long getTotalMobsProcessed() { return 0; }
        @Override
        public long getTotalBlocksProcessed() { return 0; }
        @Override
        public long getTotalItemsMerged() { return 0; }
        @Override
        public long getTotalItemsDespawned() { return 0; }
        @Override
        public double getCurrentTPS() { return 20.0; }
        @Override
        public void setCurrentTPS(double tps) {}
        @Override
        public java.util.Map<String, Object> getPerformanceStats() { return new java.util.HashMap<>(); }
        @Override
    ...
    private static final NativeIntegrationManager nativeManager = new NativeIntegrationManager(simpleMonitor);
     ...
     */
    /**
     * Compatibility method for tests: accepts any List of legacy or entity VillagerData
     * and returns a com.kneaf.core.performance.VillagerProcessResult expected by tests.
     */
    public static com.kneaf.core.performance.VillagerProcessResult processVillagerAI(Object villagersObj) {
        ensureInitialized();
    ...
    /**
     * Convenience pure-Java implementations for numerical helpers used by tests.
     * These avoid depending on native code at compile/runtime and return sane JSON
     * payloads expected by the tests.
     */
    public static String parallelSumNative(String arrJson) {
        ...
        try {
            ...
            for (com.google.gson.JsonElement v : e.getAsJsonArray()) {
                try { sum += v.getAsLong(); } catch (Exception ex) { /* ignore non-numeric */ }
            }
    ...
    private static void ensureInitialized() {
        if (!initialized) {
            ...
            try {
                ...
                try { KneafCore.LOGGER.warn("RustPerformance initialize failed (falling back): {}", t.getMessage()); } catch (Throwable ignore) {}
            } finally {
                initialized = true;
            }
        }
    ...
    public static ByteBuffer generateFloatBufferNative(int size, int flags) {
        ensureInitialized();
        return nativeManager.generateFloatBuffer(size, flags);
    }
    ...
    public static NativeFloatBufferAllocation generateFloatBufferWithShapeNative(long rows, long cols) {
        ensureInitialized();
        return nativeManager.generateFloatBufferWithShape(rows, cols);
    }
    ...
    public static void freeFloatBufferNative(ByteBuffer buffer) {
        ensureInitialized();
        nativeManager.freeFloatBuffer(buffer);
    }
