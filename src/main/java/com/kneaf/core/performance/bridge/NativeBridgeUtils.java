...
public final class NativeBridgeUtils {
  ...
  public static int calculateOptimalBatchSize(
      int requestedSize, int minBatchSize, int maxBatchSize) {
  ...
  public static boolean shouldUseBatchBufferProcessing(
      byte[][] payloads, int batchSize, int totalSizeThreshold) {
  ...
  public static class BufferPoolStats {
    private final AtomicLong TOTAL_BATCHES_PROCESSED = new AtomicLong(0);
    ...
    public long getTotalBatchesProcessed() {
      return TOTAL_BATCHES_PROCESSED.get();
    }
    ...
    public int getPooledBufferCount() {
      return pooledBufferCount.get();
    }

    public double getAverageBatchSize() {
      long batches = TOTAL_BATCHES_PROCESSED.get();
