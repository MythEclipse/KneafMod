...
public class BatchProcessor {
    ...
    private void processBatchOptimized() throws OptimizedProcessingException {
        ...
        if (bridgeProvider.isNativeAvailable() && batch.size() >= NativeBridgeUtils.calculateOptimalBatchSize(1, 25, 200)) {
            try {
    ...
    private ItemProcessResult processItemEntitiesDirect(List<ItemEntityData> items) {
        ...
        try {
            ...
            // JSON fallback
            String jsonInput = new com.google.gson.Gson().toJson(Map.of(PerformanceConstants.ITEMS_KEY, items));
            ...
            if (NativeBridgeUtils.isValidJsonResult(jsonResult)) {
                ...
                monitor.recordItemProcessing(items.size(), (int)parseResult.getMergedCount(), 
    ...
    private MobProcessResult processMobAIDirect(List<MobData> mobs) {
        ...
        try {
            ...
            if (bridgeProvider.isNativeAvailable()) {
                ...
                if (NativeBridgeUtils.isValidNativeResult(resultBytes)) {
                    ByteBuffer resultBuffer = ByteBuffer.wrap(resultBytes).order(java.nio.ByteOrder.LITTLE_ENDIAN);
                    List<MobData> updatedMobs =
            ...
            if (NativeBridgeUtils.isValidJsonResult(jsonResult)) {
                ...
                monitor.recordMobProcessing(mobs.size(), parseResult.getDisableList().size(), 
    ...
    private List<Long> getBlockEntitiesToTickDirect(List<BlockEntityData> blockEntities) {
        ...
        try {
            ...
            // JSON fallback
            String jsonInput = new com.google.gson.Gson().toJson(Map.of(
                PerformanceConstants.TICK_COUNT_KEY, tickCount++, 
