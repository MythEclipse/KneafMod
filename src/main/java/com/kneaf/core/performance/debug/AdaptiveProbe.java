...
public class AdaptiveProbe {
  public static void main(String[] args) {
    ...
    System.out.println(
        "Current avgTps=" + String.format("%.2f", tps) + " tickDelayMs=" + tickDelay);
    ...
    System.out.println(
        "  targetTickTimeMs=" + (BASE_TARGET_TICK_TIME_MS * (20.0 / Math.max(0.1, tps))));
    System.out.println(
        "  optimizationThreshold="
            + (int) Math.max(1, 25 * (tps >= 20.0 ? 1.0 : Math.max(0.4, tps / 20.0))));

    // BatchProcessor-like calculations
    int batchSize =
        com.kneaf.core.performance.core.PerformanceConstants.getAdaptiveBatchSize(tps, tickDelay);
    long batchTimeout =
        com.kneaf.core.performance.core.PerformanceConstants.getAdaptiveBatchTimeoutMs(
            tps, tickDelay);
    int batchSleep =
        com.kneaf.core.performance.core.PerformanceConstants.getAdaptiveBatchProcessorSleepMs(tps);
