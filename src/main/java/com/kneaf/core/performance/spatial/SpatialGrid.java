 ...
 * maintains player positions per cell to reduce distance calculation complexity from O(M) to O(log
...
public class SpatialGrid {
  ...
  public SpatialGrid() {
    this(
        com.kneaf.core.performance.core.PerformanceConstants.getAdaptiveCellSize(
  ...
  public void updatePlayer(PlayerData player) {
    ...
    PlayerData oldPlayer = playerPositionCache.get(player.getId());
    if (oldPlayer != null) {
  ...
  /**
   * Find nearby players within a radius of the given position. Returns players sorted by distance
   * (closest first).
   */
  ...
  private void addNearbyPlayersFromCell(
      Set<PlayerDistance> nearbyPlayers,
      long cellKey,
      double x,
      double y,
      double z,
  ...
  /**
   * Find the minimum squared distance to any player. This is optimized for the common case where we
   * just need to know if an entity is within a certain distance of any player.
   */
  ...
  /**
   * Check if a player position has changed significantly and should be updated. This supports
   * incremental updates for better performance.
   */
  public boolean shouldUpdatePlayer(PlayerData player) {
    PlayerData cachedPlayer = playerPositionCache.get(player.getId());
    ...
    double dy = player.getY() - cachedPlayer.getY();
    double dz = player.getZ() - cachedPlayer.getZ();
    double distanceSquared = dx * dx + dy * dy + dz * dz;
  ...
  public GridStats getStats() {
    ...
    int maxPlayersPerCell = gridCells.values().stream().mapToInt(Set::size).max().orElse(0);
  ...
  private void addPlayerToGrid(PlayerData player) {
    long cellKey = getCellKeyForPosition(player.getX(), player.getZ());
  ...
  public static class GridStats {
    ...
    public GridStats(
        int totalCells, int totalPlayers, int maxPlayersPerCell, double avgPlayersPerCell) {
    ...
    public int maxPlayersPerCell() {
      return maxPlayersPerCell;
    }

    public double avgPlayersPerCell() {
      return avgPlayersPerCell;
    }

    @Override
    public String toString() {
      return String.format(
          "GridStats{cells=%d, players=%d, maxPerCell=%d, avgPerCell=%.2f}",
