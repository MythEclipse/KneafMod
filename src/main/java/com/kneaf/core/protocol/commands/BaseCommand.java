...
public abstract class BaseCommand implements CommandExecutor {
  ...
  protected BaseCommand(
      String commandName, String description, int requiredPermissionLevel, String usage) {
   ...
   */
  protected abstract int executeCommand(CommandContext<CommandSourceStack> context)
      throws Exception;
  ...
  public int handleError(CommandContext<CommandSourceStack> context, Exception error) {
    ...
    if (error instanceof ProtocolException) {
      ...
      errorMessage =
          String.format(
              "§cCommand '%s' failed: %s (Error Code: %d)",
              commandName, protocolError.getMessage(), protocolError.getErrorCode());
    } else if (error instanceof IllegalArgumentException) {
      errorMessage =
          String.format(
              "§cInvalid arguments for command '%s': %s", commandName, error.getMessage());
    } else {
      errorMessage = String.format("§cCommand '%s' failed: %s", commandName, error.getMessage());
    }
   ...
   */
  protected void sendSuccessFormatted(
      CommandContext<CommandSourceStack> context, String format, Object... args) {
   ...
   * @param args the format arguments
   */
  protected void sendFailureFormatted(
      CommandContext<CommandSourceStack> context, String format, Object... args) {
    String message = String.format(format, args);
