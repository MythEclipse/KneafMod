...
public class ChunkCacheCommand extends BaseCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        LiteralArgumentBuilder<CommandSourceStack> builder = Commands.literal(COMMAND_NAME)
            ...
            .then(Commands.literal("stats")
                .executes(context -> mainCommand.executeStats(context)))
            .then(Commands.literal("clear")
                .executes(context -> mainCommand.executeClear(context)))
            .then(Commands.literal("maintenance")
                .executes(context -> mainCommand.executeMaintenance(context)))
            .then(Commands.literal("backup")
                .executes(context -> mainCommand.executeBackup(context)))
            .then(Commands.literal("config")
                .then(Commands.literal("capacity")
                    .then(Commands.argument(ARG_SIZE, IntegerArgumentType.integer(1, 10000))
                        .executes(context -> mainCommand.executeSetCapacity(context, IntegerArgumentType.getInteger(context, ARG_SIZE)))))
                .then(Commands.literal(ARG_POLICY)
                    .then(Commands.argument(ARG_POLICY, StringArgumentType.string())
                        .executes(context -> mainCommand.executeSetEvictionPolicy(context, StringArgumentType.getString(context, ARG_POLICY))))))
    ...
    private int executeStats(CommandContext<CommandSourceStack> context) {
        ...
        for (Map.Entry<String, ChunkStorageManager> entry : storageManagers.entrySet()) {
            ...
            if (stats.isEnabled()) {
                ...
                sendSuccessFormatted(context, pattern,
        ...
        getLogger().logMetrics("chunk_stats",
            java.util.Map.of("worlds_count", storageManagers.size()),
    ...
    private int executeClear(CommandContext<CommandSourceStack> context) {
        ...
        getLogger().logMetrics("clear_cache",
    ...
    private int executeMaintenance(CommandContext<CommandSourceStack> context) {
        ...
        for (Map.Entry<String, ChunkStorageManager> entry : storageManagers.entrySet()) {
            ...
            try {
                ...
                sendSuccessFormatted(context, "§cMaintenance failed for world '%s': %s", worldName, e.getMessage());
                // Log error but continue with other worlds
                getLogger().logError("maintenance", e, ProtocolUtils.generateTraceId(),
        ...
        getLogger().logMetrics("maintenance",
    ...
    private int executeBackup(CommandContext<CommandSourceStack> context) {
        ...
        for (Map.Entry<String, ChunkStorageManager> entry : storageManagers.entrySet()) {
            ...
            try {
                ...
                backupCount++;
                sendSuccessFormatted(context, "§aBackup created for world '%s' at '%s'", worldName, backupPath);
            } catch (Exception e) {
                sendSuccessFormatted(context, "§cBackup failed for world '%s': %s", worldName, e.getMessage());
                getLogger().logError("backup", e, ProtocolUtils.generateTraceId(),
                    java.util.Map.of("world", worldName));
            }
        }
    ...
    private int executeSetCapacity(CommandContext<CommandSourceStack> context, int capacity) {
        ...
        getLogger().logMetrics("set_capacity",
    ...
    private int executeSetEvictionPolicy(CommandContext<CommandSourceStack> context, String policy) {
        ...
        if (!policy.equalsIgnoreCase("LRU") && !policy.equalsIgnoreCase("Distance") && 
            ...
            sendSuccessFormatted(context, 
                "§cInvalid eviction policy '%s'. Valid policies: LRU, Distance, Hybrid", policy);
            return 0;
        }
        
        sendSuccessFormatted(context, "§aEviction policy set to §f%s §afor all worlds", policy);
        
        getLogger().logMetrics("set_eviction_policy",
            java.util.Map.of("new_policy", policy.equalsIgnoreCase("LRU") ? 1 :
                                           policy.equalsIgnoreCase("Distance") ? 2 : 3),
            ProtocolUtils.generateTraceId());
    ...
    private int executeHelp(CommandContext<CommandSourceStack> context) {
        String helpMessage = createHelpMessage() + "\n" +
            ...
            "§b  config " + ARG_POLICY + " <LRU|Distance|Hybrid> §7- Set eviction policy\n" +
