...
public class CommandRegistry {
    ...
    try {
      for (RegisteredCommand registeredCommand : commands.values()) {
        ...
        logger.logOperationComplete(
            "register_command", "registry", traceId, 0, true, Map.of("command", commandName));
      }
      ...
      logger.logError("register_dispatcher", e, traceId, Map.of("registry", registryName));
  ...
  private int executeCommand(CommandExecutor command, CommandContext<CommandSourceStack> context) {
    ...
    try {
      ...
      if (!command.validateArguments(context)) {
        ...
        logger.logWarning("execute_command", errorMsg, traceId, Map.of("command", commandName));
      ...
      // Log error
      logger.logError(
          "execute_command", e, traceId, Map.of("command", commandName, "duration", duration));
  ...
  public void clear() {
    ...
    logger.logOperationComplete(
        "clear_registry", "registry", traceId, 0, true, Map.of("registry", registryName));
  }
