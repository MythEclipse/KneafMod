...
/**
 * Enhanced protocol processor with comprehensive support for multiple protocol formats,
 * improved error handling, structured logging, and protocol versioning.
 */
public class EnhancedProtocolProcessor implements ProtocolHandler<Object, Object> {
    ...
    public Object processBinary(Object input) throws Exception {
        ...
        ProtocolContext context = ProtocolContext.builder()
        ...
        return processWithContext(input, context, inputData -> {
            try {
    ...
    public Object processWithFallback(Object input) throws Exception {
        ...
        logger.logOperationStart("process_with_fallback", detectedFormat, traceId,
        ...
        try {
            ...
            switch (detectedFormat) {
                ...
                default:
                    ...
                    logger.logWarning("process_with_fallback", 
                        "Unknown format '" + detectedFormat + "', falling back to JSON", 
                        traceId, Map.of("detected_format", detectedFormat));
                    result = processJson(input);
                    break;
            }
            
            logger.logOperationComplete("process_with_fallback", detectedFormat, traceId,
                context.getElapsedTime(), true, Map.of("result_type", result.getClass().getSimpleName()));
            
            return result;
            
        } catch (Exception e) {
            logger.logError("process_with_fallback", e, traceId,
     ...
     */
    private Object processWithContext(Object input, ProtocolContext context,
                                    java.util.function.Function<Object, Object> processor) throws Exception {
        ...
        try {
            ...
            if (context.getValidator() != null) {
                ...
                ProtocolValidator<Object> typedValidator = (ProtocolValidator<Object>) context.getValidator();
            ...
            logger.logOperationComplete(operation, format, traceId, context.getElapsedTime(), true,
                Map.of("result_type", result != null ? result.getClass().getSimpleName() : "null"));
            
            return result;
            
        } catch (TimeoutException e) {
            ProtocolException timeoutError = ProtocolException.timeout(operation, defaultTimeoutMs, traceId);
            ...
            if (e instanceof ProtocolException) {
                ...
                ProtocolException wrappedError = ProtocolException.serializationFailed(
    ...
    private Object executeJsonProcessing(Object input) throws Exception {
        ...
        if (!ProtocolUtils.isValidJson(jsonString)) {
            throw ProtocolException.validationFailed("Invalid JSON format", ProtocolConstants.FORMAT_JSON, 
    ...
    public <T, R> ProtocolResult<R> processWithFallback(
        ...
        try {
            ...
            if (binaryResult != null && binaryResult.length > 0) {
                ...
                logger.logOperationComplete(operationName, ProtocolConstants.FORMAT_BINARY, traceId, 0, true,
                    Map.of("result_type", result.getClass().getSimpleName()));
                return new ProtocolResult<>(result, null, true);
            }
        } catch (Exception e) {
            logger.logWarning(operationName, "Binary processing failed, trying JSON fallback", traceId,
        ...
        try {
            ...
            if (jsonResult != null && !jsonResult.isEmpty()) {
                ...
                logger.logOperationComplete(operationName, ProtocolConstants.FORMAT_JSON, traceId, 0, true,
                    Map.of("result_type", result.getClass().getSimpleName()));
                return new ProtocolResult<>(result, null, true);
            }
        } catch (Exception e) {
            logger.logWarning(operationName, "JSON processing also failed, returning fallback", traceId,
                Map.of("error", e.getMessage()));
        }
        
        // Return fallback result
        logger.logOperationComplete(operationName, "fallback", traceId, 0, true,
            Map.of("fallback_used", true));
