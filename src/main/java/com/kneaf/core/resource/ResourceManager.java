...
/**
 * Unified resource management abstraction that provides consistent lifecycle management
 * for all resources throughout the codebase.
 * 
 * This class eliminates duplicate resource management patterns by providing a centralized
...
public class ResourceManager implements Closeable {
    ...
    private static class ResourceWrapper {
        ...
        private final AtomicReference<ResourceState> state = new AtomicReference<>(ResourceState.INITIALIZED);
        ...
        void initialize() throws Exception {
            if (!state.compareAndSet(ResourceState.INITIALIZED, ResourceState.STARTED)) {
                throw new IllegalStateException("Resource " + resource.getResourceName() + " is already initialized");
            }
            resource.initialize();
        }
        ...
        void start() throws Exception {
            if (state.get() != ResourceState.STARTED) {
                throw new IllegalStateException("Resource " + resource.getResourceName() + " is not in STARTED state");
            }
            resource.start();
        }
        ...
        boolean isHealthy() {
            ...
            try {
                ...
                LOGGER.warn("Health check failed for resource {}: {}", resource.getResourceName(), e.getMessage());
                return false;
            }
        }
    ...
    public void registerResource(ManagedResource resource) throws Exception {
        ...
        try {
            ...
            throw new KneafCoreException(KneafCoreException.ErrorCategory.RESOURCE_MANAGEMENT, 
                                       "registerResource", 
                                       "Failed to register resource: " + resourceName, e);
        }
    }
    ...
    public void startResource(String resourceName) throws Exception {
        ...
        try {
            ...
            throw new KneafCoreException(KneafCoreException.ErrorCategory.RESOURCE_MANAGEMENT, 
                                       "startResource", 
                                       "Failed to start resource: " + resourceName, e);
        }
    }
    ...
    public void stopResource(String resourceName) throws Exception {
        ...
        try {
            ...
            throw new KneafCoreException(KneafCoreException.ErrorCategory.RESOURCE_MANAGEMENT, 
                                       "stopResource", 
                                       "Failed to stop resource: " + resourceName, e);
        }
    }
    ...
    public void startAll() throws Exception {
        ...
        if (!exceptions.isEmpty()) {
            KneafCoreException aggregated = new KneafCoreException(
    ...
    public static class ResourceLifecycleEvent {
        ...
        public ManagedResource getResource() { return resource; }
        public ResourceManager getSource() { return source; }
    }
    ...
    public static class ResourceHealthReport {
        ...
        public Map<String, ResourceState> getStateStatus() { return stateStatus; }
        public boolean isHealthy() { return healthStatus.values().stream().allMatch(Boolean::booleanValue); }
        public boolean isHealthy(String resourceName) { return Boolean.TRUE.equals(healthStatus.get(resourceName)); }
    }
    ...
    public static class ResourceStatistics {
        ...
        public long getTotalUptimeMs() { return totalUptimeMs; }
