...
/**
 * Test cases for RustDatabaseAdapter. Handles native library loading failures gracefully by
...
public class RustDatabaseAdapterTest {
  ...
  static {
    try {
      ...
      System.err.println(
          "Rust native library or class not available for tests: "
  ...
  void testNativeLibraryAvailability() {
    ...
    if (nativeLibraryAvailable && adapter != null) {
      System.out.println(
          "SUCCESS: Native library is available - all RustDatabaseAdapter tests can run");
    } else {
      System.out.println(
          "INFO: Native library is not available - RustDatabaseAdapter tests are being skipped gracefully");
    }
