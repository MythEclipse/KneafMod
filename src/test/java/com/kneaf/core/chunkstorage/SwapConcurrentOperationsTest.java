...
/**
 * Comprehensive testing for concurrent swap operations.
 * Tests thread safety, concurrent access patterns, and performance under load.
 */
public class SwapConcurrentOperationsTest {
    ...
    void testConcurrentSwapOutOperations() throws Exception {
        ...
        System.out.println("✓ Concurrent swap out completed: " + successfulSwaps + "/" + numChunks + " successful");
        
        // Verify statistics
        SwapManager.SwapManagerStats finalStats = swapManager.getStats();
        assertTrue(finalStats.getTotalOperations() >= initialOperations + successfulSwaps,
    ...
    void testConcurrentSwapInOperations() throws Exception {
        ...
        System.out.println("✓ Concurrent swap in completed: " + successfulSwaps + "/" + numChunks + " successful");
    ...
    void testMixedConcurrentSwapOperations() throws Exception {
        ...
        System.out.println("✓ Mixed concurrent swap completed: " + successfulSwaps + "/" + numChunks + " successful");
        
        // Verify statistics
        SwapManager.SwapManagerStats finalStats = swapManager.getStats();
        assertTrue(finalStats.getTotalOperations() >= initialStats.getTotalOperations() + successfulSwaps,
                  "Total operations should increase");
        assertEquals(0, finalStats.getActiveSwaps(), "No active swaps should remain");
    ...
    void testConcurrentBulkSwapOperations() throws Exception {
            ...
            CompletableFuture<Integer> future = CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    SwapManager.SwapOperationType operationType = 
                        (groupIndex % 2 == 0) ? SwapManager.SwapOperationType.SWAP_OUT : 
                    ...
                    if (operationType == SwapManager.SwapOperationType.SWAP_IN) {
                        ...
                        swapManager.bulkSwapChunks(group, SwapManager.SwapOperationType.SWAP_OUT)
                                  .get(3, TimeUnit.SECONDS);
                    }
                    
                    return swapManager.bulkSwapChunks(group, operationType)
                                    .get(5, TimeUnit.SECONDS);
                } catch (Exception e) {
                    System.err.println("Error in bulk swap group " + groupIndex + ": " + e.getMessage());
                    return 0;
                }
            });
        ...
        CompletableFuture<Void> allBulks = CompletableFuture.allOf(
            bulkFutures.toArray(new CompletableFuture[0])
        );
        ...
        System.out.println("✓ Concurrent bulk swap completed: " + totalSuccessfulOperations + " successful operations");
        
        // Verify statistics
        SwapManager.SwapManagerStats finalStats = swapManager.getStats();
        assertTrue(finalStats.getTotalOperations() >= initialStats.getTotalOperations() + totalSuccessfulOperations,
                  "Total operations should increase");
        assertEquals(0, finalStats.getActiveSwaps(), "No active swaps should remain");
        ...
        for (String chunkKey : chunkKeys) {
            CompletableFuture<Boolean> future = CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    concurrentAccesses.decrementAndGet();
                    System.err.println("Error in thread-safe swap for " + chunkKey + ": " + e.getMessage());
                    return false;
                }
            });
            swapFutures.add(future);
    ...
    void testConcurrentStatisticsUpdates() throws Exception {
        ...
        for (int i = 0; i < numChunks; i++) {
            ...
            CompletableFuture<Boolean> future = CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    System.err.println("Error in concurrent stats update for " + chunkKey + ": " + e.getMessage());
                    return false;
                }
            });
            swapFutures.add(future);
        ...
        assertTrue(finalStats.getTotalSwapOuts() >= initialSwapOuts + (numChunks / 2),
                  "Swap out count should increase significantly");
        assertTrue(finalStats.getTotalSwapIns() >= initialSwapIns + (numChunks / 4),
                ...
                try {
                    switch (operationType) {
                        ...
                        default:
                            ...
                            List<String> group = new ArrayList<>();
                            group.add(chunkKey);
                            return swapManager.bulkSwapChunks(group, SwapManager.SwapOperationType.SWAP_OUT)
