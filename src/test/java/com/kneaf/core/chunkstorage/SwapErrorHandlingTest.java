...
/**
 * Comprehensive error handling and recovery testing for swap operations.
 * Tests failure scenarios, error propagation, and recovery mechanisms.
 */
public class SwapErrorHandlingTest {
    ...
    void testSwapOperationTimeoutHandling() throws Exception {
        ...
        try {
            ...
            assertTrue(stats.getFailedOperations() > initialTimeouts, "Timeout should be recorded as failed operation");
    ...
    void testDatabaseFailureRecovery() throws Exception {
        ...
        try {
            ...
            assertTrue(failureStats.getFailedOperations() > initialFailures, "Failure should be recorded");
    ...
    void testInvalidChunkKeyHandling() throws Exception {
        ...
        for (String invalidKey : invalidKeys) {
            try {
                ...
                System.out.println("✓ Invalid key '" + invalidKey + "' handled with exception: " + e.getMessage());
            }
        }
        ...
        try {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                CompletableFuture<Boolean> future = CompletableFuture.supplyAsync(() -> {
                    try {
                        if (swapOut) {
                            return concurrentErrorSwapManager.swapOutChunk(chunkKey).get(2, TimeUnit.SECONDS);
                        } else {
                            return concurrentErrorSwapManager.swapInChunk(chunkKey).get(2, TimeUnit.SECONDS);
                        }
                    } catch (Exception e) {
            ...
            System.out.println("✓ Concurrent operations completed: " + successfulOperations + "/10 successful");
    ...
    void testErrorRecoveryMechanisms() throws Exception {
        ...
        class RecoveringAdapter extends RustDatabaseAdapter {
            ...
            public CompletableFuture<Optional<byte[]>> getChunkAsync(String key) {
                ...
                if (!isHealthy && attemptCount < 3) {
                    CompletableFuture<Optional<byte[]>> future = new CompletableFuture<>();
                    future.completeExceptionally(new RuntimeException("Database recovering - attempt " + attemptCount));
                    return future;
                }
                return super.getChunkAsync(key);
            }
        ...
        try {
            ...
            CompletableFuture<Boolean> secondAttempt = recoverySwapManager2.swapOutChunk(chunkKeys.get(1));
