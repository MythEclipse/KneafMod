...
/**
 * Core test for SwapManager functionality without Minecraft dependencies. This test validates the
 * basic swap system operations and configuration.
 */
public class SwapManagerCoreTest {
  ...
  void testMemoryPressureDetection() {
    ...
    assertTrue(
        usagePercentage >= 0.0 && usagePercentage <= 1.0,
  ...
  void testSwapConfiguration() {
    ...
    assertEquals(
        1000, swapConfig.getMemoryCheckIntervalMs(), "Memory check interval should be 1000ms");
    ...
    assertEquals(
        0.95, swapConfig.getCriticalMemoryThreshold(), 0.01, "Critical threshold should be 0.95");
    assertEquals(0.85, swapConfig.getHighMemoryThreshold(), 0.01, "High threshold should be 0.85");
    assertEquals(
        0.75, swapConfig.getElevatedMemoryThreshold(), 0.01, "Elevated threshold should be 0.75");
  ...
  void testSwapManagerStatistics() {
    ...
    assertTrue(
        Stats.isEnabled(),
        "Swap manager should be enabled (config: "
            + freshConfig.isEnabled()
            + ", Stats: "
            + Stats.isEnabled()
            + ")");
