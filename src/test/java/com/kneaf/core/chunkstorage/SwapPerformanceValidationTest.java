...
/**
 * Comprehensive performance validation testing for swap operations.
 * Tests timing, throughput, latency, and performance metrics accuracy.
 */
public class SwapPerformanceValidationTest {
    ...
    void testSwapOperationTimingMetrics() throws Exception {
        ...
        System.out.println("✓ Small chunk swap out avg: " + (swapOutTimes.get("small") / 1_000_000.0) + "ms");
        System.out.println("✓ Medium chunk swap out avg: " + (swapOutTimes.get("medium") / 1_000_000.0) + "ms");
        System.out.println("✓ Large chunk swap out avg: " + (swapOutTimes.get("large") / 1_000_000.0) + "ms");
        System.out.println("✓ Small chunk swap in avg: " + (swapInTimes.get("small") / 1_000_000.0) + "ms");
        System.out.println("✓ Medium chunk swap in avg: " + (swapInTimes.get("medium") / 1_000_000.0) + "ms");
        System.out.println("✓ Large chunk swap in avg: " + (swapInTimes.get("large") / 1_000_000.0) + "ms");
    ...
    void testThroughputPerformance() throws Exception {
        ...
        CompletableFuture<Void> allSwapOuts = CompletableFuture.allOf(
            swapOutFutures.toArray(new CompletableFuture[0])
        );
        ...
        CompletableFuture<Void> allSwapIns = CompletableFuture.allOf(
            swapInFutures.toArray(new CompletableFuture[0])
        );
        ...
        assertTrue(swapOutDataThroughput > 50, "Swap out data throughput should be at least 50 KB/second");
        assertTrue(swapInDataThroughput > 50, "Swap in data throughput should be at least 50 KB/second");
    ...
    void testLatencyPerformance() throws Exception {
        ...
        double avgSwapOutLatency = swapOutLatencies.stream().mapToLong(Long::longValue).average().orElse(0.0);
        ...
        double avgSwapInLatency = swapInLatencies.stream().mapToLong(Long::longValue).average().orElse(0.0);
        ...
        assertTrue(swapOutVariance < 100_000_000_000L, "Swap out latency variance should be reasonable");
    ...
    void testPerformanceUnderLoad() throws Exception {
        ...
        assertTrue(operationsPerSecond > 10, "Should maintain at least 10 operations/second under load");
    ...
    void testPerformanceMetricsAccuracy() throws Exception {
        ...
        assertEquals(initialSwapOuts + testSwapOuts, finalStats.getTotalSwapOuts(),
                    "Swap out count should be accurate");
        assertEquals(initialSwapIns + testSwapIns, finalStats.getTotalSwapIns(),
                    "Swap in count should be accurate");
        
        // Verify timing metrics are updated
        assertTrue(finalStats.getAverageSwapOutTime() >= initialAvgSwapOutTime,
                  "Average swap out time should be updated");
        assertTrue(finalStats.getAverageSwapInTime() >= initialAvgSwapInTime,
                  "Average swap in time should be updated");
        
        // Verify timing metrics are reasonable
        assertTrue(finalStats.getAverageSwapOutTime() > 0 && finalStats.getAverageSwapOutTime() < 1000,
                  "Average swap out time should be reasonable");
        assertTrue(finalStats.getAverageSwapInTime() > 0 && finalStats.getAverageSwapInTime() < 1000,
        ...
        System.out.println("✓ Final average swap out time: " + finalStats.getAverageSwapOutTime() + "ms");
    ...
    void testBulkOperationPerformance() throws Exception {
        ...
        for (int batchSize : batchSizes) {
            ...
            long startTime = System.currentTimeMillis();
            int successfulSwaps = swapManager.bulkSwapChunks(chunkKeys, SwapManager.SwapOperationType.SWAP_OUT)
            ...
            System.out.println("✓ Batch size " + batchSize + ": " + duration + "ms (" + 
                             successfulSwaps + "/" + batchSize + " successful)");
        }
        ...
        for (int i = 1; i < batchSizes.length; i++) {
            ...
            // Larger batches should not take proportionally longer
            double timeRatio = (double) largerTime / smallerTime;
            double sizeRatio = (double) largerBatch / smallerBatch;
            
            assertTrue(timeRatio < sizeRatio * 1.5, "Bulk operations should be relatively efficient");
        }
        ...
        swapManager.bulkSwapChunks(testChunks2, SwapManager.SwapOperationType.SWAP_OUT)
