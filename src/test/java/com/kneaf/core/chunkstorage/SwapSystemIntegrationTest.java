...
/**
 * Integration test for the swap system to verify the unified virtual memory implementation.
 * This test validates the integration between SwapManager and core components.
 */
public class SwapSystemIntegrationTest {
    ...
    private void testMemoryPressureLevels(SwapManager swapManager) {
        ...
        if (usageRatio >= 0.95) {
            assertEquals(SwapManager.MemoryPressureLevel.CRITICAL, pressureLevel,
                        "Should be CRITICAL at >=95% usage");
        } else if (usageRatio >= 0.85) {
            assertEquals(SwapManager.MemoryPressureLevel.HIGH, pressureLevel,
                        "Should be HIGH at >=85% usage");
        } else if (usageRatio >= 0.75) {
            assertEquals(SwapManager.MemoryPressureLevel.ELEVATED, pressureLevel,
                        "Should be ELEVATED at >=75% usage");
        } else {
            assertEquals(SwapManager.MemoryPressureLevel.NORMAL, pressureLevel,
                        "Should be NORMAL at <75% usage");
        }
    }
}