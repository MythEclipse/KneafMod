...
class RustNativeIntegrationTest {
  ...
  void callNativeProcessItemEntitiesBinary_ifAvailable() throws Exception {
    ...
    List<ItemEntityData> items =
        List.of(new ItemEntityData(12345L, 5, 7, "minecraft:stone", 64, 10));
    ByteBuffer input = ManualSerializers.serializeItemInput(42L, items);

    // Try to call the private native method reflectively; if native lib not available, skip the
    // test
    Method nativeMethod;
    try {
      nativeMethod =
          RustPerformance.class.getDeclaredMethod(
              "processItemEntitiesBinaryNative", java.nio.ByteBuffer.class);
