...
/**
 * Simple validation test for the advanced ThreadPoolExecutor implementation Tests core threading
 * logic without Minecraft dependencies
 */
public class ThreadPoolValidationTest {
  ...
  private static void testThreadPoolCreation() {
    ...
    assert config.getMaxThreadpoolSize() >= config.getMinThreadpoolSize()
        : "Max threads should be >= min threads";
    assert config.getThreadScaleUpThreshold() > 0 && config.getThreadScaleUpThreshold() <= 1.0
        : "Scale up threshold should be 0-1";
    assert config.getThreadScaleDownThreshold() > 0 && config.getThreadScaleDownThreshold() <= 1.0
        : "Scale down threshold should be 0-1";
    assert config.getThreadScaleUpThreshold() > config.getThreadScaleDownThreshold()
        : "Scale up should be > scale down";
